...
void communicate(int connectedFileDescriptor,int* connectedFileDescriptors, int playerNum){
    
    char messageToClient[100] = {0};
    char errmsg[] = "A Game is currently going on!!\n";
    if(clientNumber < playerNum)
    {
...
void* killPlayer(void *arg){
    struct killarguments  *args= (struct killarguments*) arg;
    char msg[100] = {0};
    sprintf(msg,"PlAYER#%d stepped on you\n",args->killingPlayer);
    if(TEMP_FAILURE_RETRY(bulk_write(args->fd ,msg,100))<0) ERR("sendto");
    close(connectedFileDescriptors[args->dyingPlayer]);
    connectedFileDescriptors[args->dyingPlayer] = -1;
...
int findWinner(int playerNum)
{
    int playerCount = 0;
    for (int i = 0; i < playerNum; i++)
    {
        if(connectedFileDescriptors[i] != -1)
            playerCount++;
    }
    if(playerCount == 1)
        for (int i = 0; i < playerNum; i++)
...
void* movePlayer(void *arg){
        ...
        if(args->position + args->move > args->boardsize - 1 ||args->position + args->move < 0 )
        {
            ...
            *(args->fd) = -1;
            ...
            movethePlayer(args);
            struct killarguments *kargs;
            if((kargs=(struct killarguments*)malloc(sizeof(struct killarguments)))==NULL) ERR("malloc:");
            kargs->fd=connectedFileDescriptors[dyingPlayer];
            kargs->dyingPlayer= dyingPlayer;
            kargs->killingPlayer = args->playerNumber;
            if (pthread_create(&thread, NULL,killPlayer, (void *)kargs) != 0) ERR("pthread_create");
        ...
        while(do_work)
        {
            ...
            if((pret = pselect(fdT+1,&readyFileDescriptors,NULL,NULL,&tv,&oldmask))>0)
            {
                for(int i = 0 ; i < player_num ; ++i)
                {
                    ...
                    {
                        communicate(connectedFileDescriptor, connectedFileDescriptors,player_num);
            ...
            else if(pret == 0)
            {
                ...
                if(clientNumber == player_num && gamestartpoint == 0)
                {
                    ...
                    //randomize positions
                    srand(time(NULL));
                    int *positions = (int*)malloc(sizeof(int)*player_num);
                    int randomNumber = -1;
                    for(int i =0 ; i < player_num;)
                    {

                        randomNumber = rand() % boardsize;
                        if(sem_trywait(&boardSemaphores[randomNumber]) == -1)continue;
                        else{
                    ...
                    for (int i = 0; i < player_num; i++)
                    {
                        sem_post(&boardSemaphores[positions[i]]);
                    }
                ...
                {
                    if(TEMP_FAILURE_RETRY(bulk_write(connectedFileDescriptors[winner],"You have Won!!\n",sizeof("You have Won!!\n")))<0) ERR("sendto");
                    close(connectedFileDescriptors[winner]);
                    connectedFileDescriptors[winner] = -1;
                    clientNumber = 0;
                    gamestart = 0;
                    gamestartpoint = 0;
                }
